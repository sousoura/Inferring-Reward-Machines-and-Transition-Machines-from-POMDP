...
def parse_rm_line(line: str):
    """
    将形如:
      806,2 None 805,0 SettlementPoint -> [-0.10000789033039995, Default, -0.10000157971778081, Default]
    的一行解析成:
      [("806,2", "-0.10000789033039995"), ("None", "Default"), ("805,0", "-0.10000157971778081"), ("SettlementPoint", "Default")]
    ...
    right_part = right_part.strip()  # "[-0.10000789033039995, Default, -0.10000157971778081, Default]"

    # 去除右边的方括号
    if right_part.startswith("[") and right_part.endswith("]"):
        right_part = right_part[1:-1].strip()  # "-0.10000789033039995, Default, -0.10000157971778081, Default"
    else:
    ...
    # 现在 output_symbols = ["-0.10000789033039995", "Default", "-0.10000157971778081", "Default"]
...
def stringify_rm_line(pairs):
    """
    将 (input_symbol, output_symbol) 的列表恢复成:
      inp1 inp2 ... -> [out1, Default, out2, Default, ...]
    """
    if not pairs:
        return ""
    inputs = []
...
def step2_remove_global_consistent(all_trajectories, record_filename):
    """
    检查所有“输出 != Default”的输入符号，如果输入在所有出现的地方对应的输出都是同一个值，
...
def main():
    ...
    # 若希望保留并追加新的记录，可以注释掉以下两行
    if os.path.exists(record_file):
        os.remove(record_file)

    # 1) 读入 RM_formal_trajectories.txt
    ...
    all_trajectories = []
    for line_num, line in enumerate(traj_lines, 2):  # 从第二行开始计数
        pairs = parse_rm_line(line)
        if not pairs:
            print(f"Warning: The {line_num} line haven't been parsed.")
            continue
    ...
    # 5) 输出到 reduced_RM_trajectories.txt
    with open(out_filename, "w", encoding="utf-8") as f:
        ...
        for pairs in all_trajectories:
            if not pairs:
                ...
                # 右边用逗号+空格连接，并加上方括号
                right_str = "[" + ", ".join(outputs) + "]"
                line_str = f"{left_str} -> {right_str}"
                f.write(line_str + "\n")

    print(f"Generated: {out_filename}")
    print(f"Recording file: {record_file}")
